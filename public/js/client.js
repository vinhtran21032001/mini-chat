...
var socket = io();
    ...
    btnCreate.onclick = function() {
        ...
        btnCreateRoom.onclick = function() {
            ...
            //1.1 nhan danh sach thanh vien tra ve va render ra ui
            socket.on('list-member', renderListMember)
            
            // 1.2 su kien log out room
            ...
            socket.on('alert-userout', name => {
                $('.alert').text("");
                $('.alert').append(`${name} vừa rời khỏi phòng`);
                setTimeout(function() {
                   $('.alert').text("")
                }, 2000);
            })

            // su kien chat cua user
